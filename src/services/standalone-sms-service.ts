...
export class StandaloneSMSService {
  ...
  async sendOTP(phoneNumber: string, otp: string): Promise<{ success: boolean; message: string }> {
    try {
      ...
      // Send actual SMS via Twilio - with proper error handling
      try {
        // Dynamically import Twilio to avoid dependency issues
...
export const standaloneSMSService = new StandaloneSMSService();