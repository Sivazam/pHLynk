      ...
      try {
        ...
        if (firebaseUser) {
          ...
          if (userDoc.exists()) {
            ...
            // For wholesaler admins, check tenant status (but proceed regardless)
            ...
            if (userData.roles.includes('WHOLESALER_ADMIN')) {
              ...
              // Fetch tenant document to check status (but continue even if not found)
              try {
                const tenantDoc = await getDoc(doc(db, COLLECTIONS.TENANTS, userData.tenantId));
                if (tenantDoc.exists()) {
                  const tenantData = tenantDoc.data();
                  tenantStatus = tenantData.status || 'ACTIVE';

                  // Only block if not active
                ...
                console.warn('Could not fetch tenant status, continuing:', tenantError);
              }
            }

            // For line workers, proceed regardless (they're always active)
            if (userData.roles.includes('LINE_WORKER')) {
              if (!userData.tenantId) {
                console.error('User missing tenantId:', firebaseUser.uid);
                updateProgress(90, 'Account configuration error...');
                await new Promise(resolve => setTimeout(resolve, 150));
            ...
            try {
              ...
              initializeFCM(fcmUserId, fcmUserType).then(fcmToken => {
                if (fcmToken) {
                  ...
                  console.warn('⚠️ FCM initialization failed for wholesaler/line worker user:', {
                    tenantId: userData.tenantId,
                    userId: firebaseUser.uid
                  });
                }
              }).catch(error => {
                console.error('❌ FCM initialization error for wholesaler/line worker user:', {
                  tenantId: userData.tenantId,
                  userId: firebaseUser.uid,
                  error: error instanceof Error ? error.message : 'Unknown error'
                });
              });
              console.log('✅ FCM initialization enabled for wholesalers/line workers - storing tokens in wholesalers collection');
            } catch (error) {
              console.error('❌ Failed to initialize FCM for wholesaler/line worker user:', {
                tenantId: userData.tenantId,
                error: error instanceof Error ? error.message : 'Unknown error'
              });
            }
