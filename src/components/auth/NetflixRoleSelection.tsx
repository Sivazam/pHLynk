...
export function NetflixRoleSelection({ onRoleSelect, onBack }: NetflixRoleSelectionProps) {
  ...
  const profileVariants: Variants = {
    hidden: { opacity: 0, scale: 0.7, y: 60, rotateX: 15 },
    visible: {
      opacity: 1,
      scale: 1,
      y: 0,
    ...
    selected: {
      ...
      y: -18,
      rotateX: -8,
      transition: { type: 'spring', stiffness: 250, damping: 18, duration: 0.5 },
    },
  };
  ...
  const iconVariants: Variants = {
    ...
    visible: {
      ...
      rotate: 0,
      opacity: 1,
      transition: { type: 'spring', stiffness: 180, damping: 12, delay: 0.3, duration: 0.6 },
    },
    hover: {
      scale: 1.15,
      rotate: 8,
  ...
  const glowVariants: Variants = {
    hidden: { opacity: 0, scale: 0.6, filter: 'blur(0px)' },
    visible: {
      opacity: 1,
      scale: 1,
      filter: 'blur(20px)',
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-black flex flex-col font-sans">
        ...
        <header className="relative z-50">
          <div className="flex items-center justify-between px-4 sm:px-6 py-4">
            <div className="flex items-center space-x-2">
              <Image src="/logoMain.png" alt="PharmaLync" width={28} height={28} className="rounded" />
              <span className="text-xl sm:text-2xl font-bold text-white">PharmaLync</span>
            </div>
        ...
        <main className="flex-1 flex flex-col items-center px-4 sm:px-6 pb-12 sm:pb-16">
          <div className="w-full max-w-6xl mx-auto">
            <motion.div initial={{ opacity: 0, y: -30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1 }}>
              ...
              <p className="text-gray-400 text-center mb-12">Select your access level to continue to your PharmaLync dashboard</p>
            </motion.div>
            ...
            >
              {roles.map((role) => {
                ...
                return (
                  ...
                  >
                    <motion.div variants={glowVariants} className={`absolute inset-0 rounded-full ${role.shadowColor} blur-xl`} />
                    <div className={`relative w-28 h-28 sm:w-32 sm:h-32 rounded-full bg-gradient-to-br ${role.color} flex items-center justify-center shadow-2xl`}>
                      <motion.div variants={iconVariants} className="text-white">
                        <Icon className="w-10 h-10" />
                      </motion.div>
                    </div>
                    <motion.div variants={textVariants} className="mt-4 text-center text-gray-300 group-hover:text-white">
                      <h3 className="font-semibold">{role.title}</h3>
                      <p className="text-sm text-gray-400">{role.description}</p>
                    </motion.div>
                    {isHovering && (
                      <motion.div initial={{ opacity: 0, scale: 0.6 }} animate={{ opacity: 1, scale: 1 }} className="absolute -top-3 -right-3">
            ...
            <section className="max-w-3xl mx-auto mt-20 px-4">
              ...
              <Accordion type="single" collapsible className="space-y-4">
                {[
                  { q: 'What is PharmaLync?', a: 'PharmaLync is a digital platform that helps wholesalers, line workers, and retailers manage and track pharmaceutical payments securely with OTP-based verification.' },
                  { q: 'Who should use PharmaLync?', a: 'Wholesaler Admins, Line Workers, Retailers, and Super Admins. Each role has its own dashboard and responsibilities.' },
                  { q: 'How are payments verified?', a: 'Payments are verified using a secure 6-digit OTP sent via SMS. Retailers confirm the OTP to complete the transaction.' },
                  { q: 'Can I use PharmaLync on mobile?', a: 'Yes. PharmaLync is designed mobile-first so line workers and retailers can access features easily on their phones.' },
                  { q: 'What happens if my internet connection is lost?', a: 'Line workers can still collect payments offline. Once the connection is restored, the data automatically syncs.' },
                  { q: 'How do retailers track their outstanding payments?', a: 'Retailers can log in to their dashboard to view outstanding amounts, invoices, and full payment history.' },
                  { q: 'Is my data secure?', a: 'Yes. PharmaLync uses OTP verification, encrypted communication, audit logs, and strict role-based access control to keep your data safe.' },
                  { q: 'How can I get support?', a: 'You can contact your wholesaler admin for account-related issues or reach PharmaLync support at 9014882779.' },
                ].map((faq, idx) => (
                  <AccordionItem key={idx} value={`item-${idx}`}>
                    <AccordionTrigger className="text-white hover:text-red-500 transition-colors">{faq.q}</AccordionTrigger>
                    <AccordionContent>
                      <motion.p
        ...
        {/* Footer */}
        <footer className="border-t border-gray-800 px-4 sm:px-6 py-6">
          <div className="max-w-6xl mx-auto text-gray-500 text-xs sm:text-sm space-y-3 text-center">
            <p>
              Questions? <a href="tel:9014882779" className="text-blue-600 hover:underline">9014882779</a>
            </p>
