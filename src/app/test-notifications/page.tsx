    ...
    try {
      ...
      if (response.ok) {
        if (result.configured) {
          toast.success('FCM v1 is properly configured!');
          addTestResult('FCM v1 Configuration', 'success', `Project: ${result.projectId}, Email: ${result.clientEmail}`);
        } else {
          toast.info('FCM v1 not fully configured');
          addTestResult('FCM v1 Configuration', 'warning', `Missing: ${result.missing?.join(', ') || 'Unknown'}`);
        }
      } else {
        toast.error('FCM v1 configuration check failed');
        addTestResult('FCM v1 Configuration', 'error', result.error || 'Unknown error');
      }
    } catch (error) {
  ...
  const testFCMV1Direct = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Handle expected FCM token validation errors
        if (result.error && result.error.includes('not a valid FCM registration token')) {
          toast.info('FCM v1 API working correctly');
          addTestResult('FCM v1 Direct', 'success', 'FCM v1 API is working - token validation working (expected for test token)');
        } else if (result.error && result.error.includes('not configured')) {
          toast.info('FCM v1 not configured');
          addTestResult('FCM v1 Direct', 'warning', `FCM v1 needs configuration: ${result.details || 'Unknown'}`);
        } else {
  ...
  const testFCMNotification = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (result.error && result.error.includes('not a valid FCM registration token')) {
          toast.info('FCM v1 API working correctly');
          addTestResult('FCM v1 Test', 'success', 'FCM v1 API is working - token validation working (expected for test token)');
        } else if (result.error && result.error.includes('not properly configured')) {
          toast.info('FCM v1 not configured');
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <Button 
              onClick={testFCMConfiguration}
              disabled={isLoading}
              variant="outline"
            >
              {isLoading ? 'Testing...' : 'Test FCM v1 Config'}
            </Button>
            <Button 
