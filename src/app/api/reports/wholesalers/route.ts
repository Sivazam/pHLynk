  ...
  try {
    ...
    try {
      ...
      if (!retailerSnapshot.empty) {
        ...
        // Fixed: proper logic for tenantIds extraction
        ...
        // Try searching by profile.phone field
        console.log('🔄 Trying search by profile.phone field')
        const profilePhoneQuery = query(retailersRef, where('profile.phone', '==', phone))
        const profilePhoneSnapshot = await getDocs(profilePhoneQuery)

        if (!profilePhoneSnapshot.empty) {
          const retailerDoc = profilePhoneSnapshot.docs[0]
          retailer = { id: retailerDoc.id, ...retailerDoc.data() }
          // Fixed: proper logic for tenantIds extraction
          tenantIds = retailer.tenantIds || (retailer.tenantId ? [retailer.tenantId] : [])
          console.log('✅ Retailer found via profile.phone:', {
            id: retailer.id,
            name: retailer.name,
            phone: retailer.profile?.phone,
            tenantIds: tenantIds
          })
        } else {
